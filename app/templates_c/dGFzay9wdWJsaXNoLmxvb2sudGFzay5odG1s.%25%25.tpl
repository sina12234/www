...
<section>
	<div class="container main-center">
                ...
                <div class="look-task-content">
                    ...
                    <div id="task-result" class="task-result">
                        <% if(thumb.length>0){ thumb.forEach(function(item){ %>
                            <div class="img-box">
                                <img src="<%= item.src_big%>" data-original="<%= item.src_big%>" alt="">
                            </div>
                        <% }) }  %>
                    </div>
                    <!-- 附件 -->
            ...
            <script type="template" id="student-info" >
                ...
                <table class="table-grid submit" data-page="<%= data.commitData.totalPage%>">
                    ...
                    <% for(var i = 0 ; i < commitItems.length ; i++ ) { %>
                        <tr>
                            ...
                            <td><%= commitItems[i].last_updated%></td>
                ...
                <% } %>
                <% if(data.noCommit) { var noCommitPage = data.noCommitData.page ; var noCommitItems = data.noCommitData.data; if( !data.commit  )  { %>
                ...
                <% } %>
                    ...
                    <tr>
                        <td ><%= noCommitItems[i].user_info.real_name%></td>
                        <td>————</td>
                        <td>————</td>
                        <td class="blue-link">
                            <a href="javascript:;" class="blue-link task-message" data-uid="<%= noCommitItems[i].user_info.user_id%>">催交</a>
                        </td>
                    </tr>
                    <% } %>
                    </tbody>
        ...
        $.ajax('/task/publishTask/taskDetail',{
            ...
            success:function (data) {
                ...
                if(result.code==200){
                    ...
                    if(data['noCommitList'] && data['noCommitList']['data'].length>0){
                    ...
                    //复制链接 插件不支持事件委托
                    $('#pt-work-detail .copy-link .link').zclip({
                        path:'<?php echo utility_cdn::js("/assets_v2/js/jquery/ZeroClipboard.swf"); ?>',
                        copy:function () {
                            return location.host+'/task.commitTask.studentCommitTask?taskId='+taskId;
                        },
                        afterCopy:function () {
