                ...
                <dl class="order-list">
                    ...
                    <dd class="fs14">
                        <div class="col-md-11 order-item-pro">
                            ...
                            <a href="//<?php echo SlightPHP\Tpl::$_tpl_vars["v"]->subdomain; ?>/course.info.show/<?php echo SlightPHP\Tpl::$_tpl_vars["v"]->course_id; ?>" class="col-lg-12 order-item-title">
                                <span class="col-sm-20 pd0 cBlack"><?php echo SlightPHP\Tpl::$_tpl_vars["v"]->title; ?></span>
                                <div class="cGray col-sm-20 pd0 mt5">
                                    <?php echo tpl_modifier_tr('共','course.list'); ?><?php echo SlightPHP\Tpl::$_tpl_vars["v"]->sectionNum; ?><?php echo tpl_modifier_tr('章节','course.list'); ?> 
                                ...
                                <div style="float: left;width: 100%;margin-top: 5px;">
                                    <span class="cYellow1 mr10 mt5">
                                        <?php if(SlightPHP\Tpl::$_tpl_vars["v"]->price==0){; ?>
                                ...
                                <div class="mt20 c-fl">来源：<?php echo SlightPHP\Tpl::$_tpl_vars["v"]->org_subname; ?></div>
                            </a>
                        </div>
                        ...
                        <p class="col-md-5 tec">
                            <?php if(SlightPHP\Tpl::$_tpl_vars["v"]->user_id <> SlightPHP\Tpl::$_tpl_vars["orgInfo"]->user_owner_id ){; ?>
                                ...
                                <button class="but14 lh22 mt30 distriBtn resell" courseId="<?php echo SlightPHP\Tpl::$_tpl_vars["v"]->course_id; ?>" act="orgStartResell">立即引入</button>
                                <?php }elseif( (SlightPHP\Tpl::$_tpl_vars["userCourseRelations"][SlightPHP\Tpl::$_tpl_vars["v"]->course_id])){; ?>
                                <button class="but14-default lh22 mt30 distriBtn" disabled="disabled" courseId="<?php echo SlightPHP\Tpl::$_tpl_vars["v"]->course_id; ?>">已引入</button>
                                <?php }else{; ?>
                                <button class="but14 lh22 mt30 distriBtn" courseId="<?php echo SlightPHP\Tpl::$_tpl_vars["v"]->course_id; ?>">立即引入</button>
                                <?php }; ?>
                            <?php }; ?>
                        </p>
                    </dd>
...
<section id="distriInfo" class="pt40 pl35 ml" style="display:none;">
    ...
    -->
        <p class="mt30 tec">
            <button class="but14 distriRequestBtn">确定引入</button>
    ...
    $('.distriBtn').click(function() {
        ...
        $.post("/course.promote.getPromoteCourseAjax",{ courseId:courseId},function(r){
            ...
            if(r.data['price_promote']>0) {
                ...
                _html += '<input class="bor1px col-xs-5 inputpd" id="salePrice" type="text"  price_promote="'+r.data['price_promote']/100+'"  price="'+r.data['price']/100+'" /> &nbsp;元';
                _html += '</p>';
                _html += '<p class="lh36 tel">';
                _html += '<span class="cLightgray lh22" id="errInfo">请输入'+r.data['price_promote']/100+'-'+r.data['price']/100+'之间的价格</span>';
                _html += '</p>';
            }else {
            ...
            _html += '<button class="but14 distriRequestBtn" courseId="'+ r.data["course_id"]+'">确定引入</button>';
            if(r.count<=10) {
        ...
        /*$(this).removeClass('but14').addClass('but14-default');
        $(this).html('已引入');
*/
    ...
    $('.distriRequestBtn').click(function() {
        ...
        $.post("/course.resell.Add",{ courseId:courseId,salePrice:salePrice,op:'orgStartResell'},function(ret){
            if(ret.result['code']==0){
                $(".distriBtn[courseId="+courseId+"]").removeClass('but14').addClass('but14-default');
                ...
                layer.closeAll();
                layer.msg(ret.result['msg'], { icon: 1 });
            }
        },'json');
    });
}
