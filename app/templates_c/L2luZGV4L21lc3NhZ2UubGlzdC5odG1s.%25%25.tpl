...
<head>
    <meta charset="utf-8">
    <title>我的消息 - 云课 - 专业的在线学习平台</title>
    <meta name="title" content="我的消息 - 云课 - 专业的在线学习平台">
    <meta name="keywords" content="我的消息 - 云课 - Yunke K12 在线学习 直播 云课网 在线教育">
    <meta name="description" content="我的消息 - 云课(Yunke.com) -专业的K12在线学习平台。以直播授课为核心，打破时间与空间的界限，为教师高效教学、学生高效学习、家长高效管控及机构高效管理提供完美解决方案">
...
<body>
<?php echo tpl_function_part("/index.main.usernav.message"); ?>
<section class="mt20 mb20" id="wrap-msg">
    <div class="container">
        <div class="row msg-lt-menu pos-rel">
            <!--联系人-->
            <?php echo tpl_function_part("/index.user.leftMenu"); ?>
            <!-- /联系人 -->
            <!--消息列表-->
            <div class="msg-right col-sm-16 col-xs-20 pos-rel" id="msg-right">
                ...
                <div class="msg-object" id="category-title">
                    ...
                    <p href="javascript:;" class="msg-obj-title fs14" id="msg-detail-title"><?php echo tpl_modifier_tr('互动信息','message'); ?></p>
                    <a href="javascript:;" class="linkman-set-icon linkman-set category-set">
                        <!-- 设置 -->
                ...
                <div class="msg-list-title tec fs14"><?php echo tpl_modifier_tr('消息中心','message'); ?></div>
                <!-- /title -->
                <!--消息总列表-->
            ...
            <ul class="my-groups" id="my-groups">
                ...
                <li>
                    ...
                    <div class="new_set_group">
                        ...
                        <input maxlength="10" class="bor1px new_group_name c-fl" />
...
<script type="text/javascript">
   ...
   window.onload=function() {
    ...
    $(function() {
        ...
        //msg-edit
        $(".msg-list").on("mouseover",".down-arrow",function() {
...
//new group
    ...
    function DelMsg(userFrom, msgType, obj) {
        ...
        layer.confirm("<?php echo tpl_modifier_tr('确定要删除消息吗？删除后内容将不可恢复','message'); ?>",{
            ...
            title:['<?php echo tpl_modifier_tr('删除消息','message'); ?>','color:#fff;background:#00a7ef']
        }, function() {
    ...
    function TopMsg(userFrom, msgType,type){
        layer.confirm("<?php echo tpl_modifier_tr('确定','message'); ?>?",{
            btn: ['<?php echo tpl_modifier_tr('确定','message'); ?>','<?php echo tpl_modifier_tr('取消','message'); ?>'],
            title:['<?php echo tpl_modifier_tr('置顶消息','message'); ?>','color:#fff;background:#00a7ef']
        }, function() {
    ...
    function DeltBlackName(userId,groupId,obj) {
        ...
        if(grouId == -5) {
            layer.confirm("<p><?php echo tpl_modifier_tr('是否将该联系人移出黑名单','message'); ?>？</p>",{
                btn: ['<?php echo tpl_modifier_tr('确定','message'); ?>','<?php echo tpl_modifier_tr('取消','message'); ?>'],
                title:['<?php echo tpl_modifier_tr('移出黑名单','message'); ?>','color:#fff;background:#00a7ef']
            }, function() {
            ...
            layer.confirm("<p><?php echo tpl_modifier_tr('是否将该联系人删除','message'); ?>？</p>",{
                btn: ['<?php echo tpl_modifier_tr('确定','message'); ?>','<?php echo tpl_modifier_tr('取消','message'); ?>'],
                title:['<?php echo tpl_modifier_tr('移出联系人','message'); ?>','color:#fff;background:#00a7ef']
            }, function() {
    ...
    function addBlackName(userId,groupId) {
        ...
        layer.confirm("<p><?php echo tpl_modifier_tr('加入黑名单后，将无法与对方发送消息','message'); ?><?php echo tpl_modifier_tr('确定加入黑名单','message'); ?>？</p>",{
            btn: ['<?php echo tpl_modifier_tr('确定','message'); ?>','<?php echo tpl_modifier_tr('取消','message'); ?>'],
            title:['<?php echo tpl_modifier_tr('加入黑名单','message'); ?>','color:#fff;background:#00a7ef']
        }, function() {
            $.post("/message/ajax/ContactsMove",{ userId:userIdArray, groupId:groupId },function(r) {
                if(r.code) {
                    layer.msg("失败");
    ...
    function DetailLink(userFrom, msgType, obj, curr) {
        if (userFrom) {
            ...
            getDetailList(userFrom, msgType, curr);
        }
    }
    ...
    function GetChatList() {
        ...
        layer.open({
            type: 1,
            title: ['<?php echo tpl_modifier_tr('设置分组','message'); ?>','color:#fff;background:#00a7ef'],
            closeBtn: 1,
            area: ['320px','220px'],
            shadeClose: true,
            content: $('#my-groups')
...
<script id="messageListTpl" type='text/template'>
    <<#messageList>>
        <li class="msgList" onclick="DetailLink(<<userFrom>>, <<msgType>>, this)" data-name="<<msgTypeTitle>>" userid="<<userFrom>>">
            <span class="face">
