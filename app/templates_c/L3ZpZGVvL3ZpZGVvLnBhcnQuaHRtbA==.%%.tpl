...
<html>
<head>
<title>视频剪辑-<?php echo tpl_function_part('/site.main.orgname'); ?> - 云课 - 专业的在线学习平台</title>
...
<script>
var totalTime = <?php echo SlightPHP\Tpl::$_tpl_vars["totaltime"]; ?>;
...
<section class="pd30">
	<div class="container">
		<div class="col-md-20 main-center">
			...
			<p style="padding-left:18px"><?php echo SlightPHP\Tpl::$_tpl_vars["plan_info"]->title; ?> <?php echo tpl_modifier_tr(SlightPHP\Tpl::$_tpl_vars["plan_info"]->class_name,'site.index'); ?> <?php echo tpl_modifier_tr(SlightPHP\Tpl::$_tpl_vars["plan_info"]->section_name,'site.index'); ?> <?php echo SlightPHP\Tpl::$_tpl_vars["secDesc"]; ?></p>
			<!-- tab -->
            ...
            <div class="org-video-partlist col-md-5" id="partlist">
				...
				<div class="ov-partlist-content">
					<!-- 没有数据 -->
                    <?php if(empty(SlightPHP\Tpl::$_tpl_vars["segs"])){; ?>
					...
					<div class="ov-partlist-btn tac p10">
						...
						<?php if(!empty(SlightPHP\Tpl::$_tpl_vars["segsTotalTime"])){; ?>
						<p class="mt5 cGray">剪辑后视频时长为：<?php echo SlightPHP\Tpl::$_tpl_vars["segsTotalTime"]; ?></p>
						<?php }; ?>
					</div>
                    <?php }else{; ?>
					<!-- 剪辑列表 -->
					<ul class="ov-partlist ov-pointlist" id="part-list">
                        <?php foreach(SlightPHP\Tpl::$_tpl_vars["segs"] as SlightPHP\Tpl::$_tpl_vars["k"]=>SlightPHP\Tpl::$_tpl_vars["v"]){; ?>
						<li data-id=<?php echo SlightPHP\Tpl::$_tpl_vars["k"]; ?>>
							<b><?php echo tpl_modifier_tr('片段','site.course'); ?><?php echo SlightPHP\Tpl::$_tpl_vars["k"]; ?>：</b><span><?php echo SlightPHP\Tpl::$_tpl_vars["v"]['start']; ?>-<?php echo SlightPHP\Tpl::$_tpl_vars["v"]['end']; ?></span>
						</li>
                        <?php }; ?>
					</ul>
...
function addPartFunc(){
	...
	partlist.find('li').each(function(i){
		...
		if(etime <= 0){
			layer.msg('<?php echo tpl_modifier_tr("结束时间不能小于0","site.course"); ?>');
			flag = false;
			return false;
		}

		if(stime > totalTime || etime > totalTime){
			layer.msg('<?php echo tpl_modifier_tr("时间不能大于总时间","site.course"); ?>');
			flag = false;
			return false;
		}
