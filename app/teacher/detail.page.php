...
class teacher_detail extends STpl
{
    ...
    public function pageEntry()
    {
        ...
        $this->assign('teach_score',$teacher_score);
    ...
    public function pageArticle($inPath)
    {
        if ($this->tid != $this->uid) {
            utility_cache::pageCache(600); //自己不缓存
        }
    ...
    public function pageArticleDetailComment($inPath)
    {
        if (!is_numeric($inPath[4]) || !(int)($inPath[4])) {
            $commentList = [];
        } else {
            $commentList = teacher_api::getArticleComment((int)($inPath[4]));
        }

        $this->assign('commentList', $commentList);
        $this->render("/teacher/new_teac_article_detail.comment.html");
    }
...
//        }
        ...
        $path='/teacher/detail/comment/'.$inPath[3];
        if(!empty($inPath[4])){
            $path .= '/'.$inPath[4];
        }
        $ret = comment_api::teacherAverage($this->tid);
    ...
    public function pageStyle()
    {
        ...
        $this->assign('tid', $this->tid);
        $this->assign('uid', $this->uid);
        $this->assign('list', teacher_api::getImgList($this->tid,1,100));
        $this->assign('favTotal', 0);
