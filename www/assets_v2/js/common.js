		...
		this.each(function(){
			...
			function up_fn(){
				if(W_box*(n_count-1)>=-H_ul){
					...
					if(H_ul+W_box*(n_count)>W_box){
						r_ul(n_count);
						$(options.prev).removeClass("dasabled").css({
							cursor: 'pointer'
						});
					}else{
						$(options.prev).removeClass("dasabled").css({
						...
						$(options.next).addClass("dasabled").css({
							cursor: 'auto'
						});
						return false;
					}
				}else if(W_box*(n_count)>=-H_ul){
					$(options.next).addClass("dasabled").css({
						cursor: 'auto'
					});
				}
			...
			$(options.prev).on('click',function(){
				if(n_count<0){
						...
						r_ul(n_count);
					if(n_count==0){
						$(options.prev).addClass("dasabled").css({
							cursor: 'auto'
						});
					}
						$(options.next).removeClass("dasabled").css({
							cursor: 'pointer'
						});
				}else{
					$(options.prev).addClass("dasabled").css({
...
;(function($){
	$.fn.tab=function(options){
		var defaults={
			tabTop : '.tab_li',//tabclass
			cursor : 'curr',//选中样式
			tabBox : '.tbClass', //tabbox class
			oEvent : "mousemove"//事件类型
		};
		var options= $.extend(defaults,options);
		this.each(function(){
			var _this=$(this);
			var mIndex='';
			$(options.tabTop).on(options.oEvent,function(){
				mIndex=$(this).index();
				$(this).addClass(options.cursor).siblings(options.tabTop).removeClass(options.cursor);
				_this.find(options.tabBox).eq(mIndex).show().siblings(options.tabBox).hide();
			});
		});
	}
})(jQuery);
	...
	$(".nameCode").bind('blur',function(){
		...
		if(sName.length>10){
			if(!(nameReg.test(sName))){
				$(".name-error-tip").html("姓名少于10个汉字或50个英文字符，用于上课使用").show();
			}else{
