(function(factory) {
    ...
    var RULES = {
        ...
        minValue: function(value, length, errorMsg) {
            //大于
        ...
        maxValue: function(value, length, errorMsg) {
            //小于
            value=parseFloat(value);
            if (value < length) {
                return errorMsg;
        ...
        isField: function(value, length, errorMsg) {
            //小于
            value=parseFloat(value);
            if (value != length) {
                return errorMsg;
        ...
        divSelect : function (value, errorMsg, el) {
            console.log(el);
        }
    };
    ...
    Validator.prototype.showMsg = function(target, msg, status, callback) {
        ...
        //console.log(target.attr('placeholder'));
        var $msg = $context.find('.tips');
        var _other = target.attr('data-type') || '';
        $msg.remove();
        $context.removeClass('success tip error').addClass(_current+' '+_other).append('<div class="tips col-xs-20 col-sm-8"><span class="tips-icon"></span> <span class="tips-text"> ' + msg + '</span></div>');
    };
            ...
            $body.on({
                ...
                blur: function(event) {
                    ...
                    for (var i = 0; i < validLen.length; i++) {
                        ...
                        strategyAry.unshift($.trim(this.value));
                    ...
                    if (!errMsg) {
                        ...
                        setting.successTip ? validator.showMsg($this, '', 1) : $this.parent().find('.tips').remove();
                    }
