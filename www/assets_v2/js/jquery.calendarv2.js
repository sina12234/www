...
function createTabledate(year,yue){
    ...
    rilitabledele.addClass("calendar-main");
...
function formatByDate(date){
    ...
    if (date.indexOf("-") >= 0 ){
        ...
        return date.substring(0,4)+"-"+date.substring(6,5)+"-"+date.substring(8,6);
    }
}
    ...
    if (coursetimelist.indexOf(currdays) >=0 ){
        for(d in s){
            if(formatByDate(d) === formatByDate(currday)){
                ...
                if (course_status=="2"){
                    course_info +='<div class="live-class-pic col-lg-5 col-xs-10 col-md-5 col-sm-5"><a href="'+s[d][c].buttonUrl+'"><img src="'+s[d][c].thumb_med+'"><span class="live-course-time visible-xs">'+s[d][c].start_time+'</span><span class="liveing-icon">正在上课</span></a></div>';
                }else{
                    course_info += '<div class="live-class-pic col-lg-5 col-xs-10 col-md-5 col-sm-5"><a href="'+s[d][c].buttonUrl+'"><img src="'+s[d][c].thumb_med+'"><span class="live-course-time visible-xs">'+s[d][c].start_time+'</span></a></div>';
                }

                course_info +='<div class="live-class-info col-lg-10 col-xs-8"><a href="'+s[d][c].buttonUrl+'">';
        ...
        liveList.append('<div class="col-xs-20 fs14 tac" style="padding-top:60px;display:block;"><img src="/assets_v2/img/platform/pet3.png"><p>没有直播课，看看其他的吧~</p></div>');
    }
}
