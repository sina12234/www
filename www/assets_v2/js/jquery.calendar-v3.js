        ...
        for(d in s){
            if(formatByDate(d) === formatByDate(currday)){
                ...
                if (course_status=="2"){
                    ...
                    course_info += '<div class="live-class-pic col-lg-4 col-xs-10 col-md-5 col-sm-5"><a href="'+s[d][c].url+'" target="_blank"></a><img src="'+s[d][c].thumb_med+'"><span class="live-course-time visible-xs">'+s[d][c].start_time+'</span><span class="g-icon3 record-icon c-fl hidden-lg hidden-xs hidden-sm hiddne-md">录播</span>';
					if(s[d][c].type_id=='3'){
					course_info+='<span class="lineclass-icon">线下</span>';
					}
					course_info+='</div>';
                }
				...
				if(s[d][c].status=="1"&&s[d][c].type_id !='3'&&s[d][c].t_status=='1'){
                    course_info +='<div class="live-class-btn col-lg-3 hidden-xs col-md-3 c-fr col-sm-3"><a href="/course.plan.start/'+s[d][c].plan_id+'" class="entry-btn" target="_blank">进入课堂</a></div>';
					course_info +='<p class="tec mt10 c-fl ml30"><a href="./teacher.manage.plan/'+s[d][c].plan_id+'" title="" target="_blank">备课</a> | <a href="./video.point.videoUpload/'+s[d][c].plan_id+'/1" title="" target="_blank">上传视频</a></p></li>';
                }else if(s[d][c].status=="3"&&s[d][c].t_status=="1"){
                    course_info +='<div class="live-class-btn col-lg-3 hidden-xs col-md-3 c-fr col-sm-3"><a href="/video.point.videoPart/'+s[d][c].plan_id+'/1" class="sign-btn" target="_blank">视频管理</a></div>';
					course_info +='<p class="tec mt10 c-fl" style="width:15%;"><a href="./course.plan.start/'+s[d][c].plan_id+'" title="" target="_blank">继续上课</a>';
