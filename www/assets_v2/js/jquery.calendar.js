$(function(){
    ...
    leftrightclick();
    setRight(new Date().getFullYear(),new Date().getMonth()+1,new Date().getDate());
});
...
function setRight(year,yue,day){
    year=year.toString();
    yue=yue.toString();
    day=day.toString();
    var date=new Date();
...
function setA(){
    ...
    for(var i=0;i<arr.length;i++){
        ...
        var datearr=date.split("-");
            ...
            arr[i].setAttribute("onclick","javascript:setRight("+datearr[0]+","+datearr[1]+","+datearr[2]+");");
            }
...
function leftrightclick(){
    ...
    righta.click(function(){
        ...
        if(aclick==""){
            ...
            if(new Date().getDate()>pervdays1){
                setRight(yearselect.html(),monthselect.html(),pervdays1);
                live_list_today(yearselect.html(),monthselect.html(),pervdays1);
            }else{
                setRight(yearselect.html(),monthselect.html(),new Date().getDate());
            ...
            setRight(aarr[1]+1==13?aarr[0]+1:aarr[0],aarr[1]+1==13?1:aarr[1]+1,aarr[2]);
    ...
    lefta.click(function(){
        ...
        setRight(aarr[1]-1==0?aarr[0]-1:aarr[0],aarr[1]-1==0?12:aarr[1]-1,aarr[2]);
        }
    });
...
function today(){
    ...
    createTabledate(parseInt(yearselect.html()),parseInt(monthselect.html()));
    setRight(date.getFullYear(),date.getMonth()+1,date.getDate());
}
    ...
    if (coursetimelist.indexOf(currdays) >=0 ){
        for(d in s){
            if(formatByDate(d) === formatByDate(currday)){
                ...
                if (course_status=="2"){
                    course_info +='<div class="live-class-pic col-lg-4 col-xs-10 col-md-5 col-sm-5"><a href="'+s[d][c].buttonUrl+'"><img src="'+s[d][c].thumb_med+'"><span class="live-course-time visible-xs">'+s[d][c].start_time+'</span><span class="liveing-icon">正在上课</span></a></div>';
                }else{
                    course_info += '<div class="live-class-pic col-lg-4 col-xs-10 col-md-5 col-sm-5"><a href="'+s[d][c].buttonUrl+'"><img src="'+s[d][c].thumb_med+'"><span class="live-course-time visible-xs">'+s[d][c].start_time+'</span></a></div>';
                }

                course_info +='<div class="live-class-info col-lg-11 col-xs-10"><a href="'+s[d][c].buttonUrl+'">';
                if (s[d][c].tryAndSee=="0"){
                    course_info +='<p class="title fs16">'+s[d][c].title+'</p>';
                } else {
                    course_info +='<p class="title fs16">'+s[d][c].title+'<i class="try-icon"></i></p>';
                }
                course_info +='<p class="cGray fs14">'+s[d][c].section_name+'</p>';
                course_info +='<p class="cGray fs14">'+s[d][c].class_name+'&nbsp;讲师：'+s[d][c].user_plan_name+'</p>';
				if(s[d][c].tips == 1){
					if(s[d][c].price_resell == 0){
						course_info +='<span class="cGreen fs14">免费</span></a>';
					}else{
						course_info +='<span class="cYellow fs14">￥'+s[d][c].price_resell+'</span></a>';
					}
				}else{
					if (s[d][c].fee_typ=="0" || s[d][c].price=="0"){
                    course_info +='<span class="cGreen fs14">免费</span></a>';
					} else {
						course_info +='<span class="cYellow fs14">￥'+s[d][c].price+'</span></a>';
					}
				}
                
                course_info +='</div>';
                if (s[d][c].buttonType=="1"){
                    course_info +='<div class="live-class-btn col-lg-3 hidden-xs col-md-3 c-fr col-sm-3"><a href="'+s[d][c].buttonUrl+'" class="watch-btn">观看视频</a></div>';
                }else if(s[d][c].buttonType=="2"){
                    course_info +='<div class="live-class-btn col-lg-3 hidden-xs col-md-3 c-fr col-sm-3"><a href="'+s[d][c].buttonUrl+'" class="entry-btn">进入课堂</a></div>';
                }else if(s[d][c].buttonType=="3"){
                    course_info +='<div class="live-class-btn col-lg-3 hidden-xs col-md-3 c-fr col-sm-3"><a href="'+s[d][c].buttonUrl+'" class="sign-btn">立即报名</a></div>';
                }else if(s[d][c].buttonType=="4") {
                    course_info +='<div class="live-class-btn col-lg-3 hidden-xs col-md-3 c-fr col-sm-3"><a href="'+s[d][c].buttonUrl+'" class="no-btn">不可会看</a></div>';
                }else {
                    course_info +='<div class="live-class-btn col-lg-3 hidden-xs col-md-3 c-fr col-sm-3"><a href="'+s[d][c].buttonUrl+'" class="no-btn">查看详情</a></div>';
                }
        ...
        liveList.append('<div class="col-xs-20 fs14 tac" style="padding-top:60px;padding-bottom:60px;display:block;"><img src="/assets_v2/img/platform/pet3.png"><p>没有直播课，看看其他的吧~</p></div>');
    }
}
