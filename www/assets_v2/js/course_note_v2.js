...
$(document).ready(function($) {
	var plan_id = /.+play.(\d+)/ig.exec(window.location.href)[1];
	...
	(function(){
	    ...
	    function jump(){
	    	if(isH5player()){
	    		canSeek = !H5Player.$player.currentTime?false:true;
	    	}else{
	...
	function render(op){
		return ejs.render(op.str,{ data: op.data});
	}
