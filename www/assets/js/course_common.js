...
function textResponse(data){ pyy = data;
	...
	if(data.is_teacher){
		dom = '<div title="'+data.last_updated+'" class="dhk1" user_id="' + data.user_from_id + '"><span><img src="' + thumb + '" width="24" height="24"></span><div class="jjj"></div><div class="mess"><p>老师说:<b c>' + a + '</b></p></div></div>';
	}else{
		dom = '<div title="'+data.last_updated+'" class="dhk" user_id="' + data.user_from_id + '"><span><img src="' + thumb + '" width="24" height="24"></span><div class="jjj"></div><div class="mess"><p>' + name + '说:<b c>' + a + '</b></p></div></div>';
	}
	var t = new Date();
...
function replyTextResponse(data){
	...
	var dom;
	dom = '<div title="'+data.last_updated+'" class="dhk" user_id="' + data.user_from_id + '"><span><img src="' + thumb + '" width="24" height="24"></span><div class="jjj"></div><div class="mess"><p>' + name + '说:<b reply_star class="reply_star" title="答题模式不可见">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</b><b reply_text style="display:none;" c>'+data.content.replace(_flag2urlReg, flag2img)+'</b></p></div></div>';
	var t = new Date();
	if(t - _lastDate > 60000){
		_chat_list_cache.push('<div class="xinxi"><p class="time">'+t.getHours()+':'+t.getMinutes()+'</p></div>');
	}
...
function goodResponseLater2(u){
	if(u in _info){ console.log("------good, u=["+u+"]\n");
		...
		//set student
		if(1 == o.num){
			var good = $('<div><div class="Ranking-p" uid="'+u+'"><p good2 class="dy">'+(_goodNum+1)+'</p><p><img good width="46" height="46" src="'+o.thumb+'"></p><p class="dy1" good>'+o.name+'</p><p class="dz"><b good>1</b>次</p></div><div class="clear"></div></div>');
			/*if(_goodNum){
				$("#good_list").append('<div class="clear"></div>');
			}*/
			$("#good_list").append(good);
