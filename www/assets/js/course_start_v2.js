	...
	function update_students(data){ console.log("***** total \n");pyy = data;
		for(var i=0;i<data.length;i++){
			...
			if(!_studentInfo[item.uid]){
				...
				var dom = $('<dl class="col-md-3 col-sm-4 col-xs-6"   uid="'+item.uid+'" online="'+item.live_online+'"><div class="add"style="display:none;"><h1>举手</h1></div><dt><img width="52" thumb  src="'+thumb+'" ></dt><ul class="ces"><li microtest="untest"><img src="/assets/images/microphoneun.png" alt="1" title="麦克风未测试" /></li><li microtest="success" style="display:none;"><img src="/assets/images/microphoneok.png" alt="1" title="麦克风测试ok" /></li><li microtest="fail" style="display:none;"><img src="/assets/images/microphonefail.png" alt="1" title="麦克风测试失败" /></li><li fullshow="0" ><img src="/assets/images/unfull.png" alt="2" title="未全屏" /></li><li fullshow="1" style="display:none;"><img src="/assets/images/webfull.png" alt="3" title="网页全屏" /></li><li fullshow="2" style="display:none;"><img src="/assets/images/flashfull.png" alt="3" title="视频全屏" /></li></ul><dd><h4 username class="limithis" title="'+name+'">'+name+'</h4>'+tmp1+'<p><img src="/assets/images/dz.png">点赞:<b good>0</b>次</p></dd><div class="clear"></div><div class="usercontrl" style="display:none"><div><li good="" style="padding-left:5px;"><a>点赞</a></li><li call="" style="padding-left: 5px;"><a>点名</a></li><li asking=""><a>请发言</a></li></div></div></dl>').appendTo(_students);
	...
	function msgDeal(data){
		...
		if(MessageType.text == data.type){
			textResponse(data);
						...
						console.log("举手来了");
			if(u in _currentAsk){
						console.log("举手在队列后来没触发");
			}else{
				if(u in _studentInfo){
					//var o =$('<div class="Ranking-p" user_id="'+data.user_from_id+'"> <p><img width="46px" height="46px" src="'+filecdn+data.user_from_thumb+'"></p><p class="dy1">'+data.user_from_name+'</p> <p class="dz"> </p> <p ask="1" class="js">举手中...</p> </div><div class="clear"></div>');
					/*		if(MessageType.online == data.type){
						var o =$('<div class="add" user_id="'+data.user_form_id+'"><h1>举手</h1></div>');
						console.log("在线举手");
						}else{
						var o =$('<div class="add" user_id="'+data.user_form_id+'"><h1>离线举手</h1></div>');
						console.log("离线举手");
						}
						*/	
						...
						if(_riseHand==1||_riseHand==0){
							_studentInfo[u]["student"].prependTo(_studentInfo[u]["student"].parent());
						}else{
							_studentInfo[u]["student"].insertAfter(_studentInfo[u]["student"].parent().children().eq(_riseHand-2));
						}
						_studentInfo[u]["student"].find("dt").before("<div class='add'style='display:block;' user_id='"+data.user_from_id+"'><h1 ask='1' >举手中...</h1></div>");
						console.log("举手成功");
						_currentAsk[u] = o;
			...
			console.log("开始取消");	
			if(u in _currentAsk){
				console.log("在队列中取消举手");	
				//deleteOneAsk(u);
				delete _currentAsk[u];
				console.log("在队列中取消举手delete");
				_riseHand--;
				if(_riseHand<=0){_riseHand=0;}
				_studentInfo[u]["student"].find(".add").remove();
				_studentInfo[u]["student"].insertAfter(_studentInfo[u]["student"].parent().children().eq(_inlineNum-1));
			}else{
				console.log("没有取消举手");	
			}
			...
			//		_studentInfo[u]["student"].prependTo(_studentInfo[u]["student"].parent());
			//		_studentInfo[u]["student"].find("dt").before("<div class='add'style='display:none;'><h1>举手</h1></div>");
					if(_inlineNum==1){
					_studentInfo[u]["student"].prependTo(_studentInfo[u]["student"].parent());
					}else{
					_studentInfo[u]["student"].insertAfter(_studentInfo[u]["student"].parent().children().eq(_inlineNum-2));
					}
			...
			if(u in _studentInfo){
				...
				if(_studentInfo[u]["inline"]){
					...
					_studentInfo[u]["student"].find(".add").remove();
					_studentInfo[u]["student"].appendTo(_studentInfo[u]["student"].parent());
...
/*	function speechOnly(uid){
		for(var u in _currentAsk){
			if(u != uid){ console.log("u=["+u+"] uid=["+uid+"]\n");
				var o = _currentAsk[u];
				var a = o.find("h1[ask=4],h1[ask=5]");
				if(a.length){
					_message.stop(u);
					delete _currentAsk[u];
					o.prev().detach();
...
///////////////////////////////////////////////////////////
	...
	$("#speech_search").on("click", "p[ask=3]", function(e){
		var p = $(this).parent();
...
//////////////////////////////////////////////////////////////////////
	...
	_students.on("click", "li[asking]", function(e){
		...
		if(_studentInfo[u]["inline"]){
			if(u in _currentAsk){
			//	var a = _currentAsk[u].find("h1[ask=1],h1[ask=2],h1[ask=3]");
				var a = _studentInfo[u]["student"].find("h1[ask=1],h1[ask=2],h1[ask=3]");
				if(a.length){
					var p = a.parent();
					$("[ask]", p).detach();
					p.append('<h1 ask="4" class="jss"><img src="/assets/images/fayan.png">发言中...</h1>');
				...
				_currentAsk[u] = a;
				_studentInfo[u]["student"].find("dt").before("<div class='add'style='display:block;' user_id='"+u+"'><h1 ask='4' >发言中...</h1></div>");
				_studentInfo[u]["student"].prependTo(_studentInfo[u]["student"].parent());
				speechOnly(u);
				_message.asking(u);
			}
